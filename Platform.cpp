...
FILE* server_fopen(const char* path, const char* mode)
{
    ...
    if (0 == strcmp(path, OC_SECURITY_DB_DAT_FILE_NAME))
    {
        ...
        fileptr = fopen("ctt_security.dat", mode);
        std::cout << "reading security file '_security.dat' ptr: " << fileptr << std::endl;
        return fileptr;
    }
	...
	fileptr = fopen("ctt_introspection.dat", mode);
	//fileptr = fopen("test.dat", mode);
        std::cout << "reading introspection file '_introspection.dat' ptr: " << fileptr << std::endl;
        return fileptr;
    }
    else
    {
